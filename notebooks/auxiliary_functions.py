...
def plot_function(function, bounds, dimension, points=None):
    ...
    # p1 and p2 are bounds, so R should be the value *inside* those bounds. Therefore,
    # remove the last value from the R array.
    R = R[:-1, :-1]
    ...
    cf = ax1.contourf(
        P1[:-1, :-1] + dP1 / 2.0, P2[:-1, :-1] + dP2 / 2.0, R, levels=levels, cmap=cmap
    )
...
def plot_distribution_of_designs(df):
    ...
    n_rec = len(dataframe)
    dataframe.loc[n_rec] = [
        [list(dataframe[reaction]).count(int(i)) / n_rec * 100 for i in range(n_modifications)]
        for reaction in reactions
    ]

    data = [[dataframe.iloc[-1][r][num] for r in reactions] for num in range(n_modifications)]
